...
class ProductsRepo(BaseRepoModel):
    ...
    async def add(self,data:AddProductDbSchema):
        new_product = Products(**data.model_dump(mode='json',exclude=['lui_id']))
        self.session.add(new_product)
        await self.session.flush() # Flush to get the autogenerated id
        
        from ..models.product import ProductPricingHistory
    ...
    async def update(self,data:UpdateProductDbSchema):
        ...
        if is_updated and 'price' in data_toupdate:
            from ..models.product import ProductPricingHistory
            self.session.add(ProductPricingHistory(
                product_id=data.product_id,
                price=data_toupdate['price'],
                created_by=self.cur_user_id
            ))
    ...
    async def get_pricing_history(self, product_id: str):
        ...
        stmt = (
            select(
                ...
                func.timezone("Asia/Kolkata", ProductPricingHistory.created_at).label("updated_at"),
                Users.name.label("updated_by_name")
            )
            .join(Users, Users.id == ProductPricingHistory.created_by, isouter=True)
            .where(ProductPricingHistory.product_id == product_id)
