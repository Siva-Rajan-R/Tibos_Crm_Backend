...
class DistributorsRepo(BaseRepoModel):
    ...
    async def add(self,data:CreateDistriDbSchema):
        new_distri = Distributors(**data.model_dump(mode='json',exclude=['lui_id']))
        self.session.add(new_distri)
        await self.session.flush() # Flush to get the autogenerated id
        
        from ..models.distributor import DistributorDiscountHistory
        self.session.add(DistributorDiscountHistory(
            distributor_id=new_distri.id,
            discounts=new_distri.discounts,
    ...
    async def update(self,data:UpdateDistriDbSchema):
        ...
        if is_updated and 'discounts' in data_toupdate:
            from ..models.distributor import DistributorDiscountHistory
            self.session.add(DistributorDiscountHistory(
                distributor_id=data.id,
                discounts=data_toupdate['discounts'],
                created_by=self.cur_user_id
            ))
    ...
    async def get_discount_history(self, distributor_id: str):
        ...
        stmt = (
            select(
                DistributorDiscountHistory.discounts,
                func.timezone("Asia/Kolkata", DistributorDiscountHistory.created_at).label("updated_at"),
                Users.name.label("updated_by_name")
            )
            .join(Users, Users.id == DistributorDiscountHistory.created_by, isouter=True)
            .where(DistributorDiscountHistory.distributor_id == distributor_id)
            .order_by(DistributorDiscountHistory.created_at.desc())
        )
