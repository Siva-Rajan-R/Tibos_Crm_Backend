...
class OrdersService(BaseServiceModel):
    ...
    async def update(self,data:UpdateOrderSchema):
        ...
        old_record = (await self.session.execute(select(Orders).where(Orders.id == data.order_id))).scalar_one_or_none()
        old_values = {}
        new_values = {}
        if old_record:
    ...
    async def get(self,filter:OrderFilterSchema,cursor:int=1,limit:int=10,query:str='',include_deleted:Optional[bool]=False,active:bool=False):
        ...
        return await OrdersRepo(session=self.session,user_role=self.user_role,cur_user_id=self.cur_user_id).get(cursor=cursor,limit=limit,query=query,include_deleted=include_deleted,filter=filter,active=active)
