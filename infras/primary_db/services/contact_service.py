...
class ContactsService(BaseServiceModel):
    ...
    async def update(self,data:UpdateContactSchema):
        ...
        old_record = (await self.session.execute(select(Contacts).where(Contacts.id == data.contact_id))).scalar_one_or_none()
        old_values = {}
        new_values = {}
        if old_record:
    ...
    async def search(self,query:str,offset:int=0):
        return await ContactsRepo(session=self.session,user_role=self.user_role,cur_user_id=self.cur_user_id).search(query=query,offset=offset)
