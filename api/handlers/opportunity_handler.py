...
class HandleOpportunitiesRequest:
    ...
    async def add(self,data:CreateOpportunitySchema):
        return await OpportunitiesService(session=self.session,user_role=self.user_role).add(data=data)

    @catch_errors
    async def update(self,data:UpdateOpportunitySchema):
        return await OpportunitiesService(session=self.session,user_role=self.user_role).update(data=data)

    @catch_errors
    async def delete(self, opportunity_id: str):
        return await OpportunitiesService(session=self.session,user_role=self.user_role).delete(opportunity_id=opportunity_id)
