...
class HandleOrdersRequest:
    ...
    async def get_cust_distri(self,customer_id: str):
        return await OrdersService(session=self.session,user_role=self.user_role,cur_user_id=self.cur_user_id).get_cust_distri(customer_id=customer_id)
    
    async def get_cust_prod(self,customer_id:str,distributor_id:str):
        return await OrdersService(session=self.session,user_role=self.user_role,cur_user_id=self.cur_user_id).get_cust_prod(customer_id=customer_id,distributor_id=distributor_id)
