...
class HandleOrdersRequest:
    ...
    async def add(self,data:AddOrderSchema):
        return await OrdersService(session=self.session,user_role=self.user_role).add(data=data)
    
    @catch_errors
    async def update(self,data:UpdateOrderSchema):
        return await OrdersService(session=self.session,user_role=self.user_role).update(data=data)
        

    @catch_errors    
    async def delete(self,order_id:str,customer_id:str):
        return await OrdersService(session=self.session,user_role=self.user_role).delete(order_id=order_id,customer_id=customer_id)
