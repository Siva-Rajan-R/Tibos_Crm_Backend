...
class HandleProductsRequest:
    ...
    async def add(self,data:AddProductSchema):
        return await ProductsService(session=self.session,user_role=self.user_role).add(data=data)


    @catch_errors   
    async def update(self,data:UpdateProductDbSchema):
        return await ProductsService(session=self.session,user_role=self.user_role).update(data=data)


    @catch_errors
    async def delete(self,product_id:str):
        return await ProductsService(session=self.session,user_role=self.user_role).delete(product_id=product_id)
