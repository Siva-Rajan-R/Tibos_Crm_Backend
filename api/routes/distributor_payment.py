...
async def delete(distri_payment_id:int,soft_delete:bool=Query(False),user:dict=Depends(verify_user),session:AsyncSession=Depends(get_pg_db_session)):
    return await HandleDistributorPaymentRequest(session=session,user_role=user['role'],cur_user_id=['id']).delete(distri_payment_id=distri_payment_id,soft_delete=soft_delete)

@router.put("/recover")
async def recover(distri_payment_id:int,user:dict=Depends(verify_user),session:AsyncSession=Depends(get_pg_db_session)):
