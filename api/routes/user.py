...
async def add_user(data:AddUserSchema,request:Request,bgt:BackgroundTasks,user:dict=Depends(verify_user),session=Depends(get_pg_db_session)):
    return await HandleUserRequest(session=session,user_role=user['role'],cur_user_id=user['id'],bgt=bgt,request=request).add(
        data=data
    )
...
async def get_users(request:Request,bgt:BackgroundTasks,user:dict=Depends(verify_user),session=Depends(get_pg_db_session)):
    return await HandleUserRequest(session=session,user_role=user['role'],cur_user_id=user['id'],bgt=bgt,request=request).get()
...
async def get_user_by_id(user_id:str,request:Request,bgt:BackgroundTasks,user:dict=Depends(verify_user),session=Depends(get_pg_db_session)):
    return await HandleUserRequest(session=session,user_role=user['role'],cur_user_id=user['id'],bgt=bgt,request=request).get_by_id(userid_toget=user_id)
...
async def get_users_by_role(role:UserRoles,request:Request,bgt:BackgroundTasks,session=Depends(get_pg_db_session),user:dict=Depends(verify_user)):
    return await HandleUserRequest(session=session,user_role=user['role'],cur_user_id=user['id'],bgt=bgt,request=request).get_by_role(userrole_toget=role)
...
async def update_user_role(data:UserRoleUpdateSchema,request:Request,bgt:BackgroundTasks,user:dict=Depends(verify_user),session=Depends(get_pg_db_session)):
    user=await HandleUserRequest(session=session,user_role=user['role'],cur_user_id=user['id'],bgt=bgt,request=request).update_role(user_toupdate_id=data.user_id,role_toupdate=data.role)
    return user
...
async def delete_user(user_id:str,request:Request,bgt:BackgroundTasks,soft_delete:Optional[bool]=Query(True),user:dict=Depends(verify_user),session=Depends(get_pg_db_session)):
    ic(user_id,soft_delete)
    user=await HandleUserRequest(session=session,user_role=user['role'],cur_user_id=user['id'],bgt=bgt,request=request).delete(userid_toremove=user_id,soft_delete=soft_delete)
    return user

@router.put('/recover')
async def recover_user(data:RecoverUserSchema,request:Request,bgt:BackgroundTasks,user:dict=Depends(verify_user),session=Depends(get_pg_db_session)):
    user=await HandleUserRequest(session=session,user_role=user['role'],cur_user_id=user['id'],bgt=bgt,request=request).recover(data=data)
    return user
