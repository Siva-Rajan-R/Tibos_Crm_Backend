...
async def delete_contact(contact_id:str,user:dict=Depends(verify_user),session:AsyncSession=Depends(get_pg_db_session)):
    return await ContactsCrud(
        session=session,
        user_email=user['id'],
        user_role=user['role']
...
async def get_contact_by_id(contact_id:str,user:dict=Depends(verify_user),session:AsyncSession=Depends(get_pg_db_session)):
    return await ContactsCrud(
        session=session,
        user_email=user['id'],
        user_role=user['role']
