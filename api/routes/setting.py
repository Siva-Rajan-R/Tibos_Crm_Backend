...
@router.post('/pending-invoice-alert')
async def upsert_pending_invoice_alert(data:PendingInvoiceAlertSchema,session:AsyncSession=Depends(get_pg_db_session)):
    return await SettingsService(session=session).pending_invoice_alert_upsert(data=data)
