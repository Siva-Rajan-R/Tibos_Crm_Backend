...
@router.get('')
async def get_all_distributor(user:dict=Depends(verify_user),q:str=Query(''),page:Optional[str]=Query('1'),cursor:Optional[str]=Query('1'),limit:Optional[int]=Query(30),session:AsyncSession=Depends(get_pg_db_session)):
    try:
        cursor=int(cursor)
    except:
        cursor=None
